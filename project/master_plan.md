...
this package to ≤ 1.0 × 10⁻¹⁵ while Excel is off by 2.4 × 10⁻⁹ to 1.6 × 10⁻⁷, so Excel's
Bessel functions carry about eight or nine correct digits. `IMSQRT("-1")` is `"i"` here and
`"6.12323399573677E-17+i"` in Excel, which goes through polar form and keeps the rounding
error.

The roadmap already says a disagreement is a finding only where *we* are right, and that the
~0.40% where we are not must never be reported as a workbook defect. This is the other half
of the same rule and it had not been written down: **where we are right and the file is
wrong, that is not a defect in the workbook either.** A cached value that differs from a
recomputation because Excel's own function was imprecise is a fact about Excel, and reporting
it would bury the real findings in noise generated by being correct.

**Two implementations disagreeing does not say which is wrong.** The first conformance round
recorded the Bessel differences as an accuracy problem in BusinessMath, on no evidence beyond
its being the newer of the two. A third implementation settled it in one command and reversed
the answer. Reach for the third opinion before attributing fault — scipy, a published table,
or a definitional value like J₀(0).
