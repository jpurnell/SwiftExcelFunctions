...
- **`#CALC!` does not exist.** `ExcelError` has seven cases and this is not one; a lambda
  alone in a cell is `#CALC!` in Excel. Adding it is a SwiftExcelCore change — small, but it
  lands in the same release as the `CellValue` case and should be decided with it.
- **What call depth is right?** Excel's own limit is undocumented; observation puts useful
  recursion in the low thousands. A separate counter is needed either way (§12.3).
- **Sheet-scoped lambdas.** A defined name can be scoped to one sheet. The resolver already
  takes a sheet, so this should be free — untested, and worth a case.
- **Does anything downstream strip `_xlpm.`?** `FunctionRegistry.canonical` strips `_XLL.`
  and `_XLFN.` and a third prefix would be easy to add by accident. Confirmed absent today.
